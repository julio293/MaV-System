    ...
    <div>
      <div onScroll={onScroll} style={{ display: 'flex', gap: 12, overflowX: 'auto', scrollSnapType: 'x mandatory', padding: '0 16px', scrollbarWidth: 'none' }}>
        {banks.map((b) => {
          ...
          return (
            <div key={b.id} className="balance-card" style={{ flex: '0 0 calc(100% - 32px)', scrollSnapAlign: 'center', width: "3px" }}>
              <div className="bal-row">
                ...
                <button className="iconbtn" style={{ color: '#fff' }} onClick={() => setHidden((h) => !h)} aria-label="Toggle balance">
      ...
      <div className={`screen-scroll pad-top pad-nav ${lockedOut ? 'locked-blur' : ''}`}>
        ...
        <div className="preview-bar">
          ...
          <div className="pv-chips">
            {[['default', 'Default'], ['new', 'New user'], ['power', 'Power user'], ['loggedout', 'Logged out']].map(([k, lbl]) =>
            <button key={k} className={`pv-chip ${state === k ? 'on' : ''}`} onClick={() => setState(k)}>{lbl}</button>
            )}
          </div>
        </div>
        ...
        <div className="mod" style={{ ...mod(), paddingTop: 4 }}>
            <div className="wallet-stack">
              {!isNew &&
            <button className="ws-peek" onClick={() => nav.go('card')} aria-label="Open wallet">
                  <span className="wp-brand"><img src="assets/ft-mark-1080.png" alt="" style={{ width: 16, height: 16, borderRadius: 5, objectFit: 'cover' }} onError={(e) => {e.target.style.display = 'none';}} /> fyscal × Federal Bank</span>
                  <span className="wp-hint"><Icon.nfc s={13} /> Tap to open wallet</span>
                </button>
            }
...
const NotificationCenter = ({ nav, app }) => {
  ...
  const open = (n, viaAction) => {
    ...
    if (t && t.screen) nav.go(t.screen, t.params || {});else
    if (n.link && n.link.screen) nav.go(n.link.screen, n.link.params || {});
  };
          ...
          return (
            <div key={g}>
              ...
              <div className="list-card" style={{ marginBottom: 14 }}>
                {list.map((n, i) =>
                <div key={n.id} style={{ borderTop: i ? '1px solid var(--mav-bg-secondary)' : 'none' }}>
                    <SwipeNotif n={n} onRead={read} onDelete={del} onOpen={open} />
                  </div>
                )}
              </div>
            </div>);
