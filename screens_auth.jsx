...
const Otp = ({ nav, params }) => {
  ...
  const onChange = (v) => {
    ...
    if (d.length === 6) {
      setTimeout(() => {
        if (d === '000000') setErr(true);else
        nav.go('biometric', { phone: params.phone });
      }, 280);
    }
  };
...
const SetPin = ({ nav, params, app }) => {
  ...
  const onKey = (k) => {
    ...
    if (next.length === 6) {
      setTimeout(() => {
        if (stage === 'set') {setFirst(next);setPin('');setStage('confirm');} else
        {
          if (next === first) {app.setAuthed(true);nav.reset('home');app.toast({ tone: 'success', title: 'You’re all set', desc: 'Welcome to Fyscal, ' + DATA.USER.first });} else
          {setErr(true);setTimeout(() => {setPin('');setStage('set');setFirst('');}, 700);}
        }
      }, 150);
    }
  };
